...
void analyse(UIRAnalyser *analyser, UIRModule *module, UIRValue *inst) {
  ...
  case UIRValueKind::Call: {
    UIRValue *callee = inst->call.callee;
    ...
    if (inst->call.receiver.isSome()) {
      UIRValue *receiver_inst = inst->call.receiver.get();
  ...
  case UIRValueKind::Index: {
    ...
    UIRValue *index = inst->index.index;
  ...
  case UIRValueKind::Return: {
    if (inst->parent->parent->kind == UIRValueKind::Function) {
      UIRValue *function = inst->parent->parent;
      ...
      if (inst->ret.value.isNone()) {
        ...
        UIRValue *value_inst = inst->ret.value.get();
  ...
  case UIRValueKind::Switch: {
    ...
    for (size_t i = 0; i < inst->_switch.onvals.len; i++) {
      ...
      on_val->kind = UIRValueKind::Literal;
