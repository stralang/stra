...
UIRValue *gen(UIRGen *uirgen, Node *node, Symbol *scope) {
  ...
  case NodeKind::RawString: {
    ...
    return uirgen->ctx->makeLiteral(slice_lit);
  }
  case NodeKind::Value: {
  ...
  case NodeKind::Function: {
    ...
    if (node->function.return_type == nullptr) {
      UIRValue void_ty = {.kind = UIRValueKind::Literal};
      void_ty.literal.lit_type =
          uirgen->ctx->type_cache->get({.kind = TypeKind::TypeId});
      void_ty.literal._typeid =
          uirgen->ctx->type_cache->get({.kind = TypeKind::Void});
      void_ty.result_type = void_ty.literal.lit_type;

      return_type = uirgen->ctx->make(void_ty);
    } else {
